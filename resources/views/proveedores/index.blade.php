<x-app-layout>
                    ...
                    <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden dark:bg-slate-900 dark:border-gray-700">
                        ...
                        <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-200 dark:border-gray-700">
                            ...
                            <div class="sm:col-span-1">
                                <label for="hs-proveedor-search" class="sr-only">Buscar</label>
                                <div class="relative">
                                    <input
                                    type="text"
                                    id="hs-proveedor-search"
                                    value="{{ $search ?? '' }}"
                                    onkeyup="searchProveedor(event)"
                            ...
                            @if (Auth::user()->hasPermission('new-proveedor'))
                            <a href="{{ route('proveedores.new') }}" class="py-3 px-4 flex justify-center items-center h-[2.875rem] w-[2.875rem] text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tr class="bg-white hover:bg-gray-50 dark:bg-slate-900 dark:hover:bg-slate-800">
                                    <td class="h-px w-px whitespace-nowrap align-center">
                                        <div class="block py-2 px-6">
                                            <div class="flex items-center gap-x-3">
                                                ...
                                                <div class="grow">
                                                    <span class="block text-sm font-semibold text-gray-800 dark:text-gray-200">{{ $prov->nombre }}</span>
                                                    <span class="block text-xs text-gray-500">#{{ $prov->idproveedor }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="h-px w-px whitespace-nowrap align-center">
                                        <div class="block py-2 px-6">
                                            <span class="text-sm text-gray-700 dark:text-gray-300">{{ $prov->contacto }}</span>
                                        </div>
                                    </td>

                                    <td class="h-px w-px whitespace-nowrap align-center">
                                        <div class="block py-2 px-6">
                                            <span class="text-sm text-gray-700 dark:text-gray-300">{{ $prov->telefono }}</span>
                                        </div>
                                    </td>

                                    <td class="h-px w-px whitespace-nowrap align-center">
                                        <div class="block py-2 px-6">
                                            <span class="text-sm text-gray-700 dark:text-gray-300">{{ $prov->correo }}</span>
                                        </div>
                                    </td>

                                    <td class="h-px w-px whitespace-nowrap align-center">
                                        <div class="block py-2 px-6">
                                            <span class="text-sm text-gray-700 dark:text-gray-300">{{ $prov->direccion }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="h-px w-px whitespace-nowrap align-center text-center">
                                        <div class="block py-2 px-6" x-data="{ open: false }" @click.away="open = false">
                                            ...
                                            <div x-show="open" x-transition
                                                ...
                                                @if (Auth::user()->hasPermission('edit-proveedor'))
                                                    <a href="{{ route('proveedores.edit', $prov->idproveedor) }}"
                                                ...
                                                @if (Auth::user()->hasPermission('delete-proveedor'))
                                                    <form method="POST" action="{{ route('proveedores.destroy', $prov->idproveedor) }}" class="delete-proveedor-form">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="button"
                                    ...
                                    <td colspan="6">
                                        <div class="max-w-sm w-full min-h-[300px] flex flex-col justify-center text-center items-center mx-auto px-6 py-4">
                                            ...
                                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                                                {{ __('¿Deseas crear un proveedor?') }}
                                            </p>
                                            @if (Auth::user()->hasPermission('new-proveedor'))
                                            <div class="mt-5 grid sm:flex gap-2">
                                                <a href="{{ route('proveedores.new') }}" class="py-2 px-3 inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
        ...
        document.querySelectorAll('.delete-btn').forEach(button => {
            button.addEventListener('click', function () {
                ...
                Swal.fire({
                    ...
                    text: `Se eliminará el proveedor "${nombre}" de forma permanente.`,
                    icon: 'warning',
    ...
    function searchProveedor(e) {
        if (e.key === 'Enter') {
        const q = document.getElementById('hs-proveedor-search').value.trim();
