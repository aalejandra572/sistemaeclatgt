<x-app-layout>
                    ...
                    <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden dark:bg-slate-900 dark:border-gray-700">
                        ...
                        <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-200 dark:border-gray-700">
                            ...
                            <div class="sm:col-span-1">
                                <label for="hs-categoria-search" class="sr-only">Buscar</label>
                                <div class="relative">
                                    <input
                                        type="text"
                                        id="hs-categoria-search"
                                        value="{{ $search ?? '' }}"
                                        onkeyup="searchCategoria(event)"
                            ...
                            @if (Auth::user()->hasPermission('new-categoria'))
                            <a href="{{ route('categorias.new') }}" class="py-3 px-4 flex justify-center items-center h-[2.875rem] w-[2.875rem] text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                                    ...
                                    <td class="h-px w-px whitespace-nowrap align-center">
                                        <div class="block py-2 px-6">
                                            <div class="flex items-center gap-x-3">
                                                <div class="grow">
                                                    <span class="block text-sm font-semibold text-gray-800 dark:text-gray-200">{{ $c->nombre }}</span>
                                                    <span class="block text-xs text-gray-500">#{{ $c->idcategoria }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="h-px w-px whitespace-nowrap align-center">
                                        <div class="block py-2 px-6">
                                            <span class="text-sm text-gray-700 dark:text-gray-300">
                                                {{ \Illuminate\Support\Str::limit($c->descripcion, 90) }}
                                            </span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="h-px w-px whitespace-nowrap align-center text-center">
                                        <div class="block py-2 px-6" x-data="{ open: false }" @click.away="open = false">
                                            ...
                                            <div x-show="open" x-transition
                                                ...
                                                @if (Auth::user()->hasPermission('edit-categoria'))
                                                ...
                                                @if (Auth::user()->hasPermission('delete-categoria'))
                                                    <form method="POST" action="{{ route('categorias.destroy', $c->idcategoria) }}" class="delete-categoria-form">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="button"
                                    ...
                                    <td colspan="3">
                                        <div class="max-w-sm w-full min-h-[300px] flex flex-col justify-center text-center items-center mx-auto px-6 py-4">
                                            ...
                                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                                                {{ __('¿Deseas crear una categoría?') }}
                                            </p>
                                            @if (Auth::user()->hasPermission('new-categoria'))
                                            <div class="mt-5 grid sm:flex gap-2">
                                                <a href="{{ route('categorias.new') }}" class="py-2 px-3 inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
        ...
        document.querySelectorAll('.delete-btn').forEach(button => {
            button.addEventListener('click', function () {
                ...
                Swal.fire({
                    ...
                    text: `Se eliminará la categoría "${nombre}" de forma permanente.`,
                    icon: 'warning',
    ...
    function searchCategoria(e) {
        if (e.key === 'Enter') {
            const q = document.getElementById('hs-categoria-search').value.trim();
            const base = "{{ route('categorias.index') }}";
