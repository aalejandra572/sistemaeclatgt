<x-app-layout>
                    ...
                    <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden dark:bg-slate-900 dark:border-gray-700">
                        ...
                        <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-200 dark:border-gray-700">
                            ...
                            <div class="sm:col-span-1">
                                <label for="hs-cliente-search" class="sr-only">Buscar</label>
                                <div class="relative">
                                    <input
                                        type="text"
                                        id="hs-cliente-search"
                                        value="{{ $search ?? '' }}"
                                        onkeyup="searchCliente(event)"
                            ...
                            @if (Auth::user()->hasPermission('new-cliente'))
                            <a href="{{ route('clientes.new') }}" class="py-3 px-4 flex justify-center items-center h-[2.875rem] w-[2.875rem] text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                                <svg xmlns="http://www.w3.org/2000/svg" class="lucide lucide-plus-circle flex-shrink-0 w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M8 12h8"/><path d="M12 8v8"/></svg>
                            </a>
                            @endif
                        </div>
                        <!-- End Header -->
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tr class="bg-white hover:bg-gray-50 dark:bg-slate-900 dark:hover:bg-slate-800">
                                        ...
                                        <div class="block py-2 px-6">
                                            <div class="flex items-center gap-x-3">
                                                <div class="grow">
                                                    ...
                                                    <span class="block text-xs text-gray-500">#{{ $c->idcliente }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="h-px w-px whitespace-nowrap align-center">
                                        <div class="block py-2 px-6">
                                            <span class="text-sm text-gray-700 dark:text-gray-300">
                                                {{ \Illuminate\Support\Str::limit($c->direccion, 90) ?: '—' }}
                                            </span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="h-px w-px whitespace-nowrap align-center text-center">
                                        <div class="block py-2 px-6" x-data="{ open: false }" @click.away="open = false">
                                            ...
                                            <div x-show="open" x-transition
                                                ...
                                                @if (Auth::user()->hasPermission('edit-cliente'))
                                                ...
                                                @if (Auth::user()->hasPermission('delete-cliente'))
                                                    <form method="POST" action="{{ route('clientes.destroy', $c->idcliente) }}" class="delete-cliente-form">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="button"
                                    ...
                                    <td colspan="4">
                                        <div class="max-w-sm w-full min-h-[300px] flex flex-col justify-center text-center items-center mx-auto px-6 py-4">
                                            <div class="flex justify-center items-center w-[46px] h-[46px] bg-gray-100 rounded-lg dark:bg-gray-800">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-gray-600 dark:text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                            ...
                                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                                                {{ __('¿Deseas crear un cliente?') }}
                                            </p>
                                            @if (Auth::user()->hasPermission('new-cliente'))
                                            <div class="mt-5 grid sm:flex gap-2">
                                                <a href="{{ route('clientes.new') }}" class="py-2 px-3 inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
        ...
        document.querySelectorAll('.delete-btn').forEach(button => {
            button.addEventListener('click', function () {
                ...
                Swal.fire({
                    ...
                    text: `Se eliminará el cliente "${nombre}" de forma permanente.`,
                    icon: 'warning',
    ...
    function searchCliente(e) {
        if (e.key === 'Enter') {
            const q = document.getElementById('hs-cliente-search').value.trim();
            const base = "{{ route('clientes.index') }}";
